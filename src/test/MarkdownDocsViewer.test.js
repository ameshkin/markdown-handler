...
describe('MarkdownDocsViewer', () => {
    it('renders without crashing', () => {
        render(_jsx(MarkdownDocsViewer, { tree: mockTree, content: mockContent }));
        expect(screen.getByText('Documentation')).toBeInTheDocument();
    });
    it('displays sidebar title', () => {
        render(_jsx(MarkdownDocsViewer, { tree: mockTree, content: mockContent, sidebarTitle: "My Docs" }));
        expect(screen.getByText('My Docs')).toBeInTheDocument();
    });
    it('displays tree items in sidebar', () => {
        const { container } = render(_jsx(MarkdownDocsViewer, { tree: mockTree, content: mockContent }));
    ...
    it('calls onDocClick when file is clicked', () => {
        ...
        const { container } = render(_jsx(MarkdownDocsViewer, { tree: mockTree, content: mockContent, onDocClick: handleClick }));
        ...
        const installationButton = Array.from(sidebarButtons).find(btn => btn.textContent?.includes('Installation') &&
    ...
    it('highlights current path', () => {
        const { container } = render(_jsx(MarkdownDocsViewer, { tree: mockTree, content: mockContent, currentPath: "getting-started/installation" }));
